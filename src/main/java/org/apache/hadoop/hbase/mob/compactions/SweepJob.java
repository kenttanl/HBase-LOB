...
public class SweepJob {
  ...
  static final String MOB_COMPACTION_DELAY = "hbase.mob.compaction.delay";
  ...
  private void submit(Job job, TableName tn, String familyName) throws IOException {
    ...
    Path mobCompactionTempDir =
        new Path(tempDir, MobConstants.DEFAULT_MOB_COMPACTION_TEMP_DIR_NAME);
  ...
  private void removeUnusedFiles(Job job, TableName tn, HColumnDescriptor hcd) throws IOException {
    ...
    try {
      ...
      // archive them
      Path mobStorePath =
          MobUtils.getMobFamilyPath(job.getConfiguration(), tn, hcd.getNameAsString());
      for (String archiveFileName : toBeArchived) {
        Path path = new Path(mobStorePath, archiveFileName);
        storeFiles
            .add(new StoreFile(fs, path, job.getConfiguration(), cacheConfig, BloomType.NONE));
      }
    } finally {
    ...
    if (!storeFiles.isEmpty()) {
      try {
        MobUtils.removeMobFiles(job.getConfiguration(), fs, tn, hcd.getName(), storeFiles);
